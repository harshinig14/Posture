...
const AlertsView: React.FC<AlertsViewProps> = ({ alerts, onClear }) => {
   return (
      <div className="space-y-8 animate-in fade-in duration-500">
         <div className="flex items-center justify-between">
            <h2 className="text-3xl font-bold text-slate-900">Alerts & Logs</h2>
            <button onClick={onClear} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-rose-500 transition-colors">
               Clear All Logs
            </button>
         </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                     {alerts.map((alert) => (
                        <tr key={alert.id} className="hover:bg-slate-50 transition-colors">
                           ...
                           <td className="px-6 py-4">
                              <span className="text-sm font-medium text-slate-600">{alert.duration > 0 ? `${alert.duration}s` : '—'}</span>
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                 <div className={`w-2 h-2 rounded-full ${alert.type === 'Help Request' ? 'bg-orange-500' : 'bg-indigo-400'}`} />
                                 <span className="text-sm text-slate-700">{alert.type}</span>
                              </div>
                              {alert.type === 'Help Request' && (
                                 <p className="text-xs text-orange-500 font-semibold mt-1">🚨 Caretaker: Your client needs help!</p>
                              )}
                           </td>
                           <td className="px-6 py-4">
                              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${alert.severity === PostureState.BAD ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-600 border-amber-100'
                                 }`}>
                                 {alert.severity}
                              </span>
                           </td>
